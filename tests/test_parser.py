...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("S.F., California", True),
        ("South SF, CA", True),
        # Requested nearby cities, plus representative Peninsula, East Bay,
        # South Bay, and North Bay entries from the explicit allowlist.
        ("San Mateo, CA", True),
        ("San Jose, CA", True),
        ("Santa Clara, CA", True),
        ("Berkeley, CA", True),
        ...
        # A city/short-form must be explicitly paired with California; named
        # SF regional aliases are the intentionally narrow exception above.
        ("San Francisco", False),
        ("SF", False),
        ("SF | NYC", False),
        ("SF<br>NYC", False),
...
def test_location_filter_normalizes_and_matches_the_bay_area_scope(location: str, expected: bool) -> None:
